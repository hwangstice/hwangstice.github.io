...
Here, I have a solution to inspect this further. Instead of making the sample sends connection to DNS `aaaaaaaaaaaaaaaaaaaa.kadusus.local` via **INETSIM (REMnux)**, I make it establish connection to my host via **host files** (`cmder` needs to be opened with **administrator**).
...
When I throw the command `whoami`, I get **TCP Receive** and **TCP Send**, also the output from the terminal.
