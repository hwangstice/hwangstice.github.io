...
In this article, I present my analysis of an RTF file and explain how **`CVE-2017-11882`** exploits a vulnerability in **Equation Editor** to execute malicious payloads.
...
I also briefly explain how shellcode is stored inside the RTF format. Some of you may want to understand the shellcode structure used in this type of file. Trust me, it will hurt your brain a lot. That said, I won't go too deep into that topic since it is outside the scope of this blog. 
...
According to the <a href="https://www.biblioscape.com/rtf15_spec.htm" target="_blank">RTF file documentation</a>, this is the structure you will encounter when viewing the raw contents of an RTF file.
...
This example is not an exception. The object data is stored in the **OleSaveToStream** format, which starts with an <a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-oleds/3c695db0-943f-48a0-b134-939af3b3a4ca?redirectedfrom=MSDN" target="_blank">ObjectHeader</a>, as shown below.
...
The next field is the **ClassName**, represented as a <a href="https://msdn.microsoft.com/en-us/library/dd942554.aspx" target="_blank">LengthPrefixedAnsiString</a>. In our case, the first four bytes represent the length of the string, which is `11` (`0x0B` in hexadecimal). The class name is `Equation.3` (including the null terminator byte `0x00`), specifying the usage of **Equation Editor**.
...
Since the **FormatID** indicates that we are dealing with an **EmbeddedObject**, the remaining bytes (shown below) must be decoded according to Microsoft's <a href="https://msdn.microsoft.com/en-us/library/dd942053.aspx" target="_blank">EmbeddedObject</a> specification. Note that the screenshot below only captures a small portion of the data blob.
...
Once **Equation Editor** is launched and starts processing the embedded object data, the vulnerability is triggered, leading to the execution of the attacker's payload.
...
To understand more about `CVE-2017-11882`, I recommend reading the blog from <a href="https://www.paloaltonetworks.com/unit42/about" target="_blank">Palo Alto Unit42</a> research group.
...
Now, I use a tool called `rtfobject` to know more about these OLE Objects. You can read more about it from <a href="https://github.com/decalage2/oletools/wiki/rtfobj" target="_blank">here</a>.
...
The answer is simple. **Microsoft Equation Editor** relies on an internal format called <a href="https://rtf2latex2e.sourceforge.net/MTEF3.html" target="_blank">MTEF (MathType Equation Format)</a> to store and represent equations. From a reverse engineer's perspective, any document containing an equation must use this format, meaning it should contain an **MTEF header** followed by a series of **MTEF records**.
